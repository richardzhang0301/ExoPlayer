...
public class AVProMobileVideo_GlRender
{
    ...
    public void Destroy()
    {
        ...
        if (this.m_bCanUseGLBindVertexArray)
        {
            ...
            {
                int[] handle = { this.m_iVertexArrayObject };
                GLES30.glDeleteVertexArrays(1, handle, 0);
            ...
            {
                int[] handles = { this.m_aiVertexBufferObjects[0], this.m_aiVertexBufferObjects[1] };
    ...
    public int GetGlTextureHandle()
    {
        if (!this.m_UseFastOesPath) {
            return this.m_FrameBufferTextureHandle;
        }
    ...
    public void DestroyRenderTarget()
    {
        ...
        if (this.m_FrameBufferHandle != 0)
        {
            ...
            int[] handle = { this.m_FrameBufferHandle };
        ...
        {
            int[] handle = { this.m_FrameBufferTextureHandle };
    ...
    public void StartRender()
    {
        if (this.m_FrameBufferHandle != 0)
        {
            ...
            GLES20.glGetBooleanv(3042, resultBuffer);this.m_bBlendEnabled = (resultBuffer.get(0) == 1);
            GLES20.glGetBooleanv(2929, resultBuffer);this.m_bDepthTest = (resultBuffer.get(0) == 1);
            GLES20.glGetBooleanv(2884, resultBuffer);this.m_bCullFace = (resultBuffer.get(0) == 1);
            GLES20.glGetIntegerv(35725, resultBuffer);this.m_iCurrentProgram = resultBuffer.get(0);
            GLES20.glGetIntegerv(36006, resultBuffer);this.m_iFrameBufferBinding = resultBuffer.get(0);
            GLES20.glGetIntegerv(36007, resultBuffer);this.m_iRenderBufferBinding = resultBuffer.get(0);
            GLES20.glGetBooleanv(2960, resultBuffer);this.m_bStencilEnabled = (resultBuffer.get(0) == 1);
    ...
    public long Blit(SurfaceTexture surfaceTexture, int iNumberFramesAvailable, float[] matrix)
    {
        long result = 0L;
        ...
        int textureFormat = this.m_bTextureFormat_EOS ? 36197 : 3553;
        if (this.m_bTextureFormat_EOS)
        {
            if (surfaceTexture != null)
            {
                ...
                try
                {
                    while (iNumberFramesAvailable > 0)
                    {
                        surfaceTexture.updateTexImage();
                        result = surfaceTexture.getTimestamp();

                        iNumberFramesAvailable--;
                    }
                }
                catch (IllegalStateException e)
                {
                    new StringBuilder("Failed to updateTexImage in Blit: ").append(e);
                }
                if (matrix == null)
                {
                    matrix = new float[16];
    ...
    private void LoadGlShaders_Texture2D()
    {
        ...
        this.m_FragmentShaderSource = "#version 100\n";
        this.m_FragmentShaderSource += "uniform sampler2D texture;\n";
        this.m_FragmentShaderSource += "varying highp vec2 out_uv;\n";
        this.m_FragmentShaderSource += "\n";
        this.m_FragmentShaderSource += "void main()\n";
        this.m_FragmentShaderSource += "{\n";
    ...
    private void CreateGlTexture()
    {
        ...
        if (this.m_TextureHandle > 0)
        {
            ...
            int textureFormat;
            GLES20.glBindTexture(textureFormat = this.m_bTextureFormat_EOS ? 36197 : '෡', m_TextureHandle);
    ...
    private void DestroyGlTexture()
    {
        if (this.m_TextureHandle > 0)
        {
            int[] handle = { this.m_TextureHandle };
            GLES20.glDeleteTextures(1, handle, 0);
            GLES20.glGetError();
    ...
    {
        float[] TriangleVerticesData = { -1.0F, -1.0F, 0.0F, -1.0F, 1.0F, 0.0F, 1.0F, -1.0F, 0.0F, 1.0F, 1.0F, 0.0F };

        float[] UVData = { 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.0F, 1.0F, 1.0F };
